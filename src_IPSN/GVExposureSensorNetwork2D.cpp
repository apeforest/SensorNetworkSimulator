	...
	case GVEXPOSURESENSORNETWORK2D_MODE_GPSR_INPROGRESS:
		...
		if(ret == GPSR_DONE)
		{
			...
			for(int j=0; j<NumCommunications; j++)
			{
				...
				DistanceSum += Distance2D(GetX(GPSR_Path[j]),GetY(GPSR_Path[j]),GetX(GPSR_Path[j+1]),GetY(GPSR_Path[j+1]));
			}
			ReturnVal = 0;
			Reset();
		}
...
{ // MaxExposure_Initialize
  ...
  if ((Sensors != NULL) && (NumNeighbors != NULL) && (NeighborIndex != NULL))
  {
	...
	ShortestPath = 0;
	GPSR_ShortestPath();
	MaxLength = GVEXPOSURESENSORNETWORK2D_MAXINT;
...
{ // OutputResults
  ...
  cout << Start.GetX() << GVEXPOSURESENSORNETWORK2D_SPACE
		  << Start.GetY() << GVEXPOSURESENSORNETWORK2D_SPACE
		  << End.GetX()   << GVEXPOSURESENSORNETWORK2D_SPACE
		  << End.GetY()   << GVEXPOSURESENSORNETWORK2D_SPACE
  ...
  for(i=0;i<=NumCommunications; i++)
	  cout << setw(8) << GPSR_Path[i] << GVEXPOSURESENSORNETWORK2D_SPACE;
  cout << endl;
  */
  //OutFile.close();
...
{
		Event = CoinToss( EVENTPROBABILITY );
		NumObservation = NV;
}
...
void GVExposureSensorNetwork2D::DetectEvent(void)
{
	int i = 0;
	int DecisionMade = 0;
	int CountOnes = 0;
	...
	int SentPacketSizeSeq = 0;
	float LHR = 1;

	SentPacketSizeDist = 1;
	SentPacketSizeCentral = ceil(LOGN(NumObservation+1,2));

	// simulate the hybrid detection scheme
	while(!DecisionMade && i<NumObservation)
	{
		if(Event == 1)
		...
		else if(CountZeros >= (NumObservation - N1))
		{
			DetectionResult = 0;
			DecisionMade = 1;
		}
	...
	// simulate the sequential detection scheme
	DecisionMade = 0;
	NumSampleSeq = 0;
	...
	case CENTRALIZED:
		...
		EnergyReceive = SentPacketSizeCentral * UNITENERGYRECEIVE * NumCommunications;//(NumCommunications-1);
		EnergyMonitor = NumObservation * UNITENERGYMONITOR;
		EnergyProcess = 0;
		break;
	case DISTRIBUTED:
		EnergyTransmit = SentPacketSizeDist * UNITENERGYTRANSMIT * DistanceSquareSum; //NumCommunications;
		EnergyReceive = SentPacketSizeDist * UNITENERGYRECEIVE * NumCommunications; //(NumCommunications-1);
		EnergyMonitor = NumObservation * UNITENERGYMONITOR;
		EnergyProcess = (NumObservation+1) * UNITENERGYPROCESS;
		break;
	case HYBRID:
		EnergyTransmit = SentPacketSizeHybrid * UNITENERGYTRANSMIT * DistanceSquareSum; //NumCommunications;
		EnergyReceive = SentPacketSizeHybrid * UNITENERGYRECEIVE * NumCommunications; //(NumCommunications-1);
		EnergyMonitor = NumSampleHybrid * UNITENERGYMONITOR;
		EnergyProcess = (NumSampleHybrid + 1) * UNITENERGYPROCESS;
		break;
	case SEQUENTIAL:
	default:
		EnergyTransmit = SentPacketSizeSeq * UNITENERGYTRANSMIT * DistanceSquareSum; //NumCommunications;
		EnergyReceive = SentPacketSizeSeq * UNITENERGYRECEIVE * NumCommunications; //(NumCommunications-1);
		EnergyMonitor = NumSampleSeq * UNITENERGYMONITOR;
		EnergyProcess = NumSampleSeq * UNITENERGYSEQUENTIAL;
		break;
	}
...
double GVExposureSensorNetwork2D::GetDistanceSum(void)
{
	return DistanceSum;
}

double GVExposureSensorNetwork2D::GetDistanceSquareSum(void)
{
	return DistanceSquareSum;
}
